...
void saveGF_grid(const std::string filename, Hubbard::FunctorBuildGk& Gk){
    std::ofstream saveGFGRID;
    FFT FFTObj;
    std::vector<double> Gtau(Gk._size+1,0.0);
    ...
    for (size_t k=0; k<Gk._kArr_l.size(); k++){
        ...
        for (double el : Gtau){
          saveGFGRID << el-0.5 << " "; // Should maybe have methods operator() const ...
        }
        saveGFGRID << "\n";
    }
    saveGFGRID.close();
